...
import { useBrigades, useExpenseCategories, usePeople, useSites } from '../../db/hooks'
...
export function ExpenseForm() {
  ...
  const isCompany = effectivePersonId === 'company'

  const payerOptions = [
    { value: 'company', label: t.entry.paidByCompany },
    ...people.map((p) => ({ value: String(p.id), label: personLabel(p, brigades, t) })),
  ]
  const categoryOptions = categories.map((c) => ({ value: String(c.id), label: c.name }))
  const siteOptions = [{ value: '', label: t.entry.noSite }, ...sites.map((s) => ({ value: String(s.id), label: s.name }))]

  const amountNumber = Number(amount)
  const canSave = effectivePersonId !== '' && effectiveCategoryId !== '' && Number.isFinite(amountNumber) && amountNumber > 0

  const submit = async () => {
    if (!canSave || tenantId == null) return
    const person = isCompany ? undefined : people.find((p) => String(p.id) === effectivePersonId)
    if (!isCompany && !person) return
    ...
    if (person) {
      payload.paidByPersonId = person.id!
      payload.brigadeIdSnapshot = person.brigadeId
    } else {
      payload.paidByCompany = true
    }
